...
public class GetMatchesServiceTests
{
    ...
    private static IEnumerable<FootballMatch> GetDataWithScoresAndDates()
    {
        return new[]
        {
            FootballMatch.Create(FootballTeam.Create(""), FootballTeam.Create("Manchester United"), 1, 0,
                DateTime.Now.ToUniversalTime().AddMinutes(50)),
            FootballMatch.Create(FootballTeam.Create("FC Barcelona"), FootballTeam.Create("Real Madrid"), 1, 0,
                DateTime.Now.ToUniversalTime().AddMinutes(-6)),
            FootballMatch.Create(FootballTeam.Create("Manchester United"), FootballTeam.Create("Chelsea"), 4, 0,
                DateTime.Now.ToUniversalTime().AddMinutes(-10)),
            FootballMatch.Create(FootballTeam.Create("Mexico"), FootballTeam.Create("Poland"), 4, 0,
                DateTime.Now.ToUniversalTime().AddMinutes(1)),
            FootballMatch.Create(FootballTeam.Create("France"), FootballTeam.Create("Italy"), 1, 1,
                DateTime.Now.ToUniversalTime().AddHours(1)),
            FootballMatch.Create(FootballTeam.Create("Australia"), FootballTeam.Create("Argentina"), 1, 2,
                DateTime.Now.ToUniversalTime())
        };
    }
}